...
public class FreemanTools extends JavaPlugin implements Listener {
	...
	public void playerHit(EntityDamageEvent subevent) {
		if(FreemanTools.this.getConfig().getBoolean("Freemanslap.Enable.Other")){
	...
	public void selfHit(PlayerInteractEvent event) throws InterruptedException{
		...
		Vector velocity = player.getLocation().getDirection().multiply(FreemanTools.this.getConfig().getInt("Freemanslap.FallVelocity") * -0.4);
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		if(cmd.getName().equalsIgnoreCase("blockhead")){ // If the player typed /basic then do the following...
			if(FreemanTools.this.getConfig().getBoolean("FreemanBlockHead.Enable")){
				Player player = (Player) sender;
				if(!has(player, "blockhead")) return false;//check if player has permission freemantools.slap.other
				PlayerInventory inventory = player.getInventory();
				...
				Map<Enchantment, Integer> ench = inventory.getItemInHand().getEnchantments();
				
				if (handleft == 0){
					ItemStack itemstack4 = new ItemStack(Material.AIR, handleft,damagevalue);
					inventory.setItem(slot, itemstack4);
					
				}
				else{
					ItemStack itemstack4 = new ItemStack(item, handleft,damagevalue);
					inventory.setItem(slot, itemstack4);
				...
				if (itemstack3 != null)
					if (inventory.getHelmet() != null){
						Map<Enchantment, Integer> enchHelmet = inventory.getHelmet().getEnchantments();
					{
					if (enchHelmet != null){
		...
		if(Killer != null) 
			{
				...
				if (randomInt < FreemanTools.this.getConfig().getInt("FreemanHeadDrop.DropChance") ){
